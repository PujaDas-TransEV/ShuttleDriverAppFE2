...
// type DateFilter = 'week' | 'month' | 'quarter' | 'year';
// type StatusFilter = 'all' | 'completed' | 'cancelled' | 'in_progress' | 'scheduled' | 'premature_end';
...
//   // Apply filters whenever analytics data or filter selections change
//   useEffect(() => {
//     if (analyticsData?.trips) {
//       applyFilters();
//     }
//   }, [analyticsData, selectedFilter, selectedStatus]);
...
//       if (analyticsRes.ok) {
//         // Ensure trips have created_at or start_time for date filtering
//         const tripsWithDates = (analytics.trips || []).map((trip: any) => ({
//           ...trip,
//           created_at: trip.created_at || trip.start_time || new Date().toISOString()
//         }));
//         setAnalyticsData({ ...analytics, trips: tripsWithDates });
//       }
//       if (payoutRes.ok) setPayoutData(payout);
//     } catch (err: any) {
//       console.error("Failed to fetch data:", err);
...
//     let filtered = [...analyticsData.trips];

//     // Apply date filter
//     filtered = filterByDate(filtered, selectedFilter);
...
//   // ======================
//   const filterByDate = (trips: Trip[], dateFilter: DateFilter): Trip[] => {
//     const now = new Date();
...
//       const tripDate = new Date(trip.created_at || trip.start_time || '');
...
//     // Use filtered trips for calculations
//     const trips = filteredTrips.length > 0 ? filteredTrips : (analyticsData?.trips || []);
    
//     const totalEarnings = trips.reduce((sum, trip) => sum + (trip.earning || 0), 0);
//     const totalTrips = trips.length;
//     const completedTrips = trips.filter(t => t.status === 'completed').length;
//     const cancelledTrips = trips.filter(t => t.status === 'cancelled').length;
    
//     // Payout data might not be filtered, use it as is or scale proportionally
//     const pendingAmount = payoutData?.summary?.total_pending_payout_amount ?? 0;
//     const paidAmount = payoutData?.summary?.total_paid_out_amount ?? 0;
//     const totalBookings = payoutData?.summary?.booking_count ?? 0;
...
//       premature_end: trips.filter(t => t.status === 'premature_end').length
//     };
...
//                   <select 
//                     value={selectedFilter} 
//                     onChange={(e) => setSelectedFilter(e.target.value as DateFilter)}
//                     style={styles.select}
//                   >
//                     <option value="week" style={{ color: isDarkMode ? '#FFFFFF' : '#000000', background: isDarkMode ? '#1F1F1F' : '#FFFFFF' }}>Last 7 Days</option>
//                     <option value="month" style={{ color: isDarkMode ? '#FFFFFF' : '#000000', background: isDarkMode ? '#1F1F1F' : '#FFFFFF' }}>Last 30 Days</option>
...
//                     <option value="cancelled" style={{ color: isDarkMode ? '#FFFFFF' : '#000000', background: isDarkMode ? '#1F1F1F' : '#FFFFFF' }}>❌ Cancelled</option>
//                     <option value="in_progress" style={{ color: isDarkMode ? '#FFFFFF' : '#000000', background: isDarkMode ? '#1F1F1F' : '#FFFFFF' }}>🔄 In Progress</option>
//                     <option value="scheduled" style={{ color: isDarkMode ? '#FFFFFF' : '#000000', background: isDarkMode ? '#1F1F1F' : '#FFFFFF' }}>📅 Scheduled</option>
//                     <option value="premature_end" style={{ color: isDarkMode ? '#FFFFFF' : '#000000', background: isDarkMode ? '#1F1F1F' : '#FFFFFF' }}>⏹️ Premature End</option>
//                   </select>
//                 </div>

//                 {/* Show filter summary */}
//                 <div style={styles.filterSummary}>
//                   <span style={{ color: isDarkMode ? '#9CA3AF' : '#6B7280', fontSize: '12px' }}>
//                     Showing {filteredTrips.length} trips
//                     {selectedFilter !== 'week' && ` • ${getDateRangeLabel(selectedFilter)}`}
...
//               {/* Recent Trips Table - Using Filtered Data */}
//               <div style={styles.section}>
//                 <div style={styles.sectionHeader}>
//                   <DocumentTextIcon style={{ width: '20px', height: '20px', color: '#10B981' }} />
//                   <h2 style={styles.sectionTitle}>Recent Trips</h2>
...
//                               <span style={{
//                                 ...styles.statusBadge,
//                                 background: trip.status === 'completed' ? '#10B98120' : 
//                                            trip.status === 'cancelled' ? '#EF444420' :
//                                            trip.status === 'in_progress' ? '#3B82F620' :
//                                            trip.status === 'scheduled' ? '#8B5CF620' :
//                                            '#F59E0B20',
//                                 color: trip.status === 'completed' ? '#10B981' : 
//                                        trip.status === 'cancelled' ? '#EF4444' :
//                                        trip.status === 'in_progress' ? '#3B82F6' :
//                                        trip.status === 'scheduled' ? '#8B5CF6' :
//                                        '#F59E0B'
//                               }}>
//                                 {trip.status}
//                               </span>
...
//                     <h3 style={styles.insightTitle}>Earning Opportunity</h3>
//                     <p style={styles.insightText}>
//                       Complete {Math.max(0, 10 - getAnalyticsValue('completedTrips'))} more trips this period to unlock bonus rewards!
//                     </p>
//                   </div>
//                 </div>
//               </div>
...
// // Export function - moved inside component scope
// const exportData = () => {
...
const getStyles = (isDark: boolean) => ({
  container: {
    paddingTop: '100px', // REDUCED from 80px to move header lower
    paddingLeft: '20px',
    paddingRight: '20px',
    paddingBottom: '40px',
    maxWidth: '1200px',
