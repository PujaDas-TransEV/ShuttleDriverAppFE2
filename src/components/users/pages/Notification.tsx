...
// ============================================
// SINGLETON WebSocket Manager - Ensures ONLY ONE connection
// ============================================
class WebSocketManager {
  ...
  async connect(token: string): Promise<boolean> {
    ...
    if (this.isConnected && this.ws?.readyState === WebSocket.OPEN && this.token === token) {
      console.log('✅ WebSocket already connected, reusing existing connection. Connection ID:', this.connectionId);
      return true;
    }
    ...
    return new Promise((resolve) => {
      ...
      this.ws.onopen = () => {
        ...
        // Start ping interval (send ping every 25 seconds)
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          // Handle authentication success with connection ID
          if (payload?.message === 'WebSocket authenticated successfully.') {
            console.log('🔐 WebSocket authenticated');
            return;
          }
      ...
      this.ws.onclose = (event) => {
        ...
        // Attempt reconnect after 5 seconds (only if we have a token)
  ...
  // For debugging - get connection status
  getStatus(): string {
...
// Global flag to track if WebSocket has been initialized
let wsInitialized = false;
let wsInitPromise: Promise<boolean> | null = null;
...
const NotificationsPage: React.FC = () => {
  ...
  const handleWebSocketMessage = useCallback((payload: any) => {
    // Handle ping from server
    if (payload?.type === 'ping') {
      wsManager.current.send({ type: 'pong' });
      return;
    }
  ...
  useEffect(() => {
    const initWebSocket = async () => {
      ...
      try {
        const token = await getToken();
        if (token) {
          console.log('🔌 Initializing WebSocket connection (should happen only once)...');
          ...
          unsubscribeRef.current = wsManager.current.addListener(handleWebSocketMessage);
          
          console.log('📡 WebSocket status:', wsManager.current.getStatus());
        }
      } catch (error) {
    ...
    return () => {
      ...
      // Only remove listener, DON'T disconnect WebSocket (keep for other components)
      if (unsubscribeRef.current) {
        unsubscribeRef.current();
