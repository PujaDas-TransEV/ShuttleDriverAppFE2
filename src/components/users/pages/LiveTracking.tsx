...
const DriverLiveTracking: React.FC = () => {
  ...
  // Fetch all routes when token is available and vehicle exists
  useEffect(() => {
    ...
    fetch(`${API_BASE}/driver/routes`, { 
      ...
      .then((res) => {
        if (res.status === 400) {
          setError("No routes available for this driver.");
          setErrorType('routes');
          setShowErrorModal(true);
          return [];
        }
  ...
  // Fetch route details and trips - only if vehicle exists
  const fetchRouteDetails = async (routeId: string) => {
    if (!routeId || !token || !vehicleData) {
      setRouteDetails(null);
      setTrips([]);
      return;
    }
  ...
  const ErrorModal = () => {
        ...
        return (
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              ...
              className={`px-4 py-2.5 ${colors.button} text-white rounded-xl font-medium transition-all duration-200 flex items-center justify-center gap-2 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 text-sm min-w-[140px]`}
              style={{ height: '40px' }}
              onMouseEnter={(e) => {
            ...
            <button
              onClick={() => window.location.reload()}
              className="px-4 py-2.5 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-xl font-medium transition-all duration-200 flex items-center justify-center gap-2 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 text-sm min-w-[100px]"
              style={{ height: '40px' }}
              onMouseEnter={(e) => {
                e.currentTarget.style.transform = 'translateY(-2px)';
                e.currentTarget.style.boxShadow = '0 10px 15px -3px rgba(0,0,0,0.1)';
    ...
    const getHelpText = () => {
      if (errorType === 'vehicle') {
        return '💡 Please register your vehicle first to access live tracking.';
      }
      if (errorType === 'routes') {
  ...
  return (
    <IonPage className="bg-linear-to-br from-gray-900 to-gray-950">
      ...
      <IonContent className="relative">
        ...
        <div className="relative h-screen w-full">
          {/* Map Loading State */}
          {loading && !routeDetails && !showErrorModal && (
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-gray-900/90 backdrop-blur-sm">
              <div className="text-center">
                <div className="w-16 h-16 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
                <p className="text-white font-medium">Loading map...</p>
          ...
          {/* Map Container - Only show if no error */}
          {!showErrorModal && !mapError ? (
            <MapContainer
              key={selectedRouteId || "default"}
              center={mapCenter}
              zoom={13}
          ...
          {/* Show message when error modal is open */}
          {showErrorModal && (
            <div className="absolute inset-0 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm">
              <div className="text-center p-6">
                <div className="text-6xl mb-4">📍</div>
                <p className="text-white text-lg font-semibold mb-2">No Vehicle Assigned</p>
                <p className="text-gray-400 text-sm">
                  Please register a vehicle to access live tracking
                </p>
              </div>
            </div>
          )}
          ...
          {/* Stops Count Indicator */}
          {routeDetails?.stops && routeDetails.stops.length > 0 && !showErrorModal && (
            <div className="absolute top-20 right-4 z-10 bg-black/60 backdrop-blur-sm rounded-xl px-3 py-2">
              <div className="flex items-center gap-2">
                <span className="text-white text-xs font-medium">
                  📍 {routeDetails.stops.length} Stops on Route
          ...
          <div className="absolute top-11 left-0 right-0 z-10 p-4 bg-linear-to-b from-black/60 to-transparent pointer-events-none">
            <div className="flex items-center justify-between pointer-events-auto">
              ...
              {driverPos && !locationError && !showErrorModal && (
                <div className="bg-black/50 backdrop-blur-sm rounded-full px-3 py-1.5">
                  <div className="flex items-center gap-1">
                    <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                    <span className="text-white text-xs font-medium">Live</span>
          ...
          {/* Bottom Sheet - Only show if no error */}
          {!showErrorModal && (
            <div
              className={`fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 rounded-t-3xl shadow-2xl transition-all duration-500 z-20 ${
                sheetOpen ? "h-[75%]" : "h-auto"
