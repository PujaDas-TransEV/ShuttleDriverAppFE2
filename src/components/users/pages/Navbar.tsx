...
//   // ======================
//   // Connect WebSocket for real-time notifications with duplicate prevention
//   // ======================
...
//         // Handle new notification - PREVENT DUPLICATES
...
//             // Clean up old IDs from set (keep last 100)
...
//       // Remove tokens from Capacitor Preferences
//       await removeToken();
...
//       // Still try to clear local tokens even if server call fails
//       await removeToken();
//       sessionStorage.clear();
//       history.push('/login');
//       setSidebarOpen(false);
...
const NavbarSidebar: React.FC = () => {
  ...
  const menuItems = [
    ...
    { 
      ...
      submenu: [
        { name: 'My Fines', path: '/fines', icon: DocumentTextIcon },
        { name: 'Pay Fine', path: '/pay-fine', icon: BanknotesIcon },
        { name: 'Fine History', path: '/fine-history', icon: DocumentTextIcon },
        { name: 'Dispute Fine', path: '/dispute-fine', icon: DocumentTextIcon },
      ]
    },
    { name: 'Booking', icon: RectangleStackIcon, path: '/passenger-booking' },
    { name: 'Live Tracking', icon: MapIcon, path: '/live-tracking' },
