...
//         body: JSON.stringify({ 
//           email, 
//           role: "driver"  // ✅ Added role: "driver" for send OTP
//         }),
//       });
...
//         body: JSON.stringify({ 
//           email, 
//           role: "driver"  // ✅ Added role: "driver" for resend OTP
//         }),
//       });
...
//           email, 
//           otp: finalOtpValue,
//           role: "driver"  // ✅ Added role: "driver" for verify OTP
//         }),
//       });
...
//   try {
//     // Get device info from Capacitor Device plugin
...
//     let deviceFamily = '';
//     let platform = '';
//     let browser = 'Driver App';
    
//     // Determine platform
//     if (Capacitor.getPlatform() === 'android') {
//       platform = 'Android';
//       deviceFamily = deviceInfo.model || 'Android Device';
//       deviceName = `${deviceInfo.manufacturer || 'Unknown'} ${deviceInfo.model || 'Device'}`;
//     } else if (Capacitor.getPlatform() === 'ios') {
//       platform = 'iOS';
//       deviceFamily = deviceInfo.model || 'iPhone/iPad';
//       deviceName = `${deviceInfo.model || 'Apple Device'}`;
//     } else if (Capacitor.getPlatform() === 'web') {
//       platform = 'Web';
//       browser = getBrowserName();
//       deviceFamily = 'Web Browser';
//       deviceName = `${browser} on ${getOSName()}`;
//     } else {
//       platform = 'Unknown';
//       deviceFamily = 'Unknown Device';
//       deviceName = 'Driver App';
//     }
...
//     setLoading(true);
//     try {
//       // Prepare request body with device info if available
...
//       if (deviceInfo) {
//         requestBody.device = {
//           device_name: deviceInfo.device_name || 'Driver App',
//           device_family: deviceInfo.device_family || 'Mobile Device',
//           platform: deviceInfo.platform || 'Unknown',
//           browser: deviceInfo.browser || 'Driver App'
//         };
//         console.log('Sending device info with login:', requestBody.device);
//       }
...
//         // Check for driver device limit error
//         if (data.detail?.error === 'driver_device_limit_reached') {
//           errorMsg = 'You have reached the maximum number of active logins. Please remove one active login and try again.';
//           showErrorModal('Device Limit Reached', errorMsg);
//           // Navigate to device management to remove devices
//           setTimeout(() => {
...
const Login: React.FC = () => {
  ...
  const checkProfileAndRedirect = async (token: string) => {
    try {
      ...
      // On error, redirect to profile setup as safe default
      history.replace('/profile-setup');
    }
  };
  ...
  const handleOtpSubmit = async (otpValue?: string) => {
    ...
    try {
      ...
      if (saved) {
        window.dispatchEvent(new Event(NOTIFICATION_SESSION_STARTED_EVENT));
        showNotification(`Welcome ${user?.email || 'Driver'}!`, 'success');
        
        // Check profile status and redirect accordingly
        await checkProfileAndRedirect(access_token);
      } else {
        showErrorModal('Storage Error', 'Failed to save login information');
      }
